...
echo "<pre>Setup by Aemr(www.aftika2011.tk)</pre>" > /home/vps/public_html/index.html
...
useradd -M -s /bin/false AemrHabibin
echo "AemrHabibin:$PASS" | chpasswd
...
echo -e "\e[1;33;44m[ ARBURA | 0838-513-99-153 | 745D1F66 ]\e[0m"| tee log-install.txt
echo "===============================================" | tee -a log-install.txt
echo ""  | tee -a log-install.txt
...
echo "---------------"  | tee -a log-install.txt
echo "User     : arbura"  | tee -a log-install.txt
...
echo "Timezone : Asia/Makassar"  | tee -a log-install.txt
